...
export async function proxyHls(req, res, targetUrl, headers = {}, proxyBaseUrl) {
  ...
  if (!upstream.ok && upstream.status !== 206) {
    res.status(upstream.status).json({ error: `A origem respondeu ${upstream.status}.`, code: "http_status" });
    return;
  }
...
function rewriteManifest(manifest, target, proxyBaseUrl) {
  const wrap = (value) => proxyBaseUrl + "?url=" + encodeURIComponent(new URL(value, target).toString());
