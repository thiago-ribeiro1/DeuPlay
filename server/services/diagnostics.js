...
const PATTERNS = [
  [/Name or service not known|nodename nor servname|Temporary failure in name resolution/i, DiagnosticCode.DNS_RESOLUTION_FAILED],
  [/Connection timed out|Operation timed out|timed out after|I\/O timeout/i, DiagnosticCode.TIMEOUT],
  ...
  [/Unknown decoder|Unsupported codec|Encoder not found|codec not currently supported/i, DiagnosticCode.CODEC_NOT_RECOGNIZED],
  [/Non-monotonic DTS|Application provided invalid, non monotonically increasing dts|timestamp discontinuity/i, DiagnosticCode.TIMESTAMP_ERROR],
  ...
  [/Cannot allocate memory|Resource temporarily unavailable|No space left on device/i, DiagnosticCode.RESOURCE_LIMIT],
...
export function classifyFfmpegLikeError(stderrText, exitCode) {
  ...
  if (exitCode !== undefined && exitCode !== 0) {
    return { code: DiagnosticCode.FFMPEG_EXITED, detail: `Processo encerrou com codigo ${exitCode}.` };
  }

  return { code: DiagnosticCode.UNKNOWN_ERROR, detail: "Falha nao classificada. Veja os logs para detalhes." };
}
...
export function describeCode(code) {
  const messages = {
    ...
    [DiagnosticCode.FFMPEG_NOT_INSTALLED]: "FFmpeg nao esta instalado ou FFMPEG_PATH esta incorreto.",
    [DiagnosticCode.FFPROBE_NOT_INSTALLED]: "ffprobe nao esta instalado ou FFPROBE_PATH esta incorreto.",
